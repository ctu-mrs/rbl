...
# # sim=1
# CSV_FILE = "sim2/rbl_odom_" + str(args.sim) + ".bag.csv"  
# OUTPUT_FILE = "sim2/uav_metrics_summary" + str(args.sim) + ".csv"
# DIST_THRESHOLD = 0.0   # Goal threshold [m]
...
# # -----------------------------
# # Compute inter-UAV distances (CORRECTED)
# # -----------------------------

# # Unified timestamp set for all UAV trajectories
# all_times = np.unique(
#     np.concatenate([np.asarray(v["t"]) for v in uav_groups.values()])
# )
...
# # Interpolated positions matrix: shape (num_t, num_uav, 3)
...
# # Pairwise distances: shape (num_t, num_uav, num_uav)
# dist_matrix = np.linalg.norm(
#     positions[:, :, None, :] - positions[:, None, :, :],
#     axis=-1
# )

# # Replace self-distances (0) with +inf so they don't become minimum
# dist_matrix[dist_matrix == 0] = np.inf

# # Min distance per UAV over time
# min_dist_per_uav = dist_matrix.min(axis=2)  # shape: (num_t, num_uav)

# # Convert to dict for plotting
# inter_uav_distances = {
#     uav_list[ui]: min_dist_per_uav[:, ui] for ui in range(num_uav)
# }

# # Store metrics
# for ui, uav in enumerate(uav_list):
#     metrics_list[uav]["MinDistanceToOtherUAV_m"] = float(
#         np.nanmin(min_dist_per_uav[:, ui])
#     )
...
# Parameters
# -----------------------------
CSV_FILE = "SIM_2/rbl_odom_" + str(args.sim) + ".bag.csv"
OUTPUT_FILE = "SIM_2/uav_metrics_summary" + str(args.sim) + ".csv"
